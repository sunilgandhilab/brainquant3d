...
def writePoints(filename, points, mode = "o", radius = 0.5, scale = all, offset = None):
    ...
    if h5file.get(pnt) is not None:
        del h5file[pnt]
    h5file.create_dataset(pnt, shape=(npts,1), dtype='i4', data=ts)

    if h5file.get(pnc) is not None:
        del h5file[pnc]
    h5file.create_dataset(pnc, shape=pts.shape, dtype='f8', data=pts)
